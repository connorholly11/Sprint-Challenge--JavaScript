...
console.log(tyrannosaurus.scare());
...
let universities = [];
  ...
  universities = graduates.map(function(arrayItem){
      return (`${arrayItem.university}`);
  })
universities.sort();
...
let contactInfo = [];
  ...
  contactInfo = graduates.map(function(arrayItem){
      return (`${arrayItem.first_name}, ${arrayItem.email}`)
  });
...
let lowerCase = [];
  ...
  lowerCase = zooAnimals.map(function(arrayItem){
...
let lowerPopulation = [];
  ...
  lowerPopulation = zooAnimals.filter(function(arrayItem){
      return `${arrayItem.population}` < 5;
  })
...
let populationTotal = 0;
  ...
  populationTotal = zooAnimals.reduce(function(acc, cur){
